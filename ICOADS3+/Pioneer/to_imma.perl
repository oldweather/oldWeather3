...
# Process digitised logbook data from Pioneer into
#  IMMA records.
...
unless ( defined($Year) ) { die "Usage: --year=<year>"; }
...
sub ddmmss_to_dec {
    ...
    open(DIN,sprintf "$FindBin::Bin/positions.olddates.annual/%04d.out",$Year) or die;
    while (my $Line = <DIN>) {
	chomp $Line;
	$Line =~ s/^\s+//;
	my @fields = split /\t/,$Line;
...
# Process the obs
	...
	my @fields = split /\t/,$Line;
	my $Asset=$fields[0];
	unless(defined($Dates{$Asset})) { next; }
	$Dates{$Asset} =~ /(\d\d)\D(\d\d)\D(\d\d\d\d)/;
	unless($3==$Year) { next; }   
	my $Hour =$fields[1];
    ...
    open(DOUT,sprintf ">../../imma_3+/%s.%04d.imma",$Name,$Year) or die;
    for(my $i=0;$i<scalar(@Imma);$i++) {
	$Imma[$i]->write(\*DOUT);
    }
    close(DOUT);
