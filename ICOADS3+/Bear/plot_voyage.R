...
Options<-WeatherMap.set.option(Options,'lat.min',-50)
Options<-WeatherMap.set.option(Options,'lat.max',50)
Options<-WeatherMap.set.option(Options,'lon.min',-40)
Options<-WeatherMap.set.option(Options,'lon.max',80)
...
for(year in c(1884,seq(1886,1898),1901,seq(1903,1912),1918)) {
    ...
    o<-read.fwf(file=sprintf('../../imma_3+/Bear.%d.imma',year),
                widths=c(4,2,2,4,5,6,84))
    o$Longitude<-o$V6/100
    o$Latitude<-o$V5/100

    Options$label<-sprintf("Bear %d",year)
