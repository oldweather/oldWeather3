...
if ( is.null(opt$ship) )   { stop("Ship not specified") }

# Get the observations for this ship
o<-read.fwf(file=sprintf('../imma.annual/%s.imma',opt$ship)                     
            widths=c(4,2,2,4,5,6,36,5,4,5,12,5))
...
# Get mean and spread from 3.5.1 for each ob.
...
# Output the result
fileConn<-file(sprintf("%s/obs+R",opt$ship))
