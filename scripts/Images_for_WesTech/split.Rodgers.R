# Cut the Rodgers images into individual pages in the same way as
#  done on oldWeather.

library(jpeg)

original.dir<-'/scratch/hadpb/oW3_logbooks/NARA/Rodgers'
...
for(fn in raw) {
  ...
  fn1<-sprintf("%s/%s_0.jpg",split.dir,substr(fn,9,21))
  writeJPEG(j1,fn1)
  fn2<-sprintf("%s/%s_1.jpg",split.dir,substr(fn,9,21))
  writeJPEG(j2,fn2)
}
