...
# Process digitised logbook data from Perry into
#  IMMA records.
...
# Get the positions
my %Positions;
open(DIN,"<positions_Perry.fmt") or die;
while(my $Line = <DIN>) {
    chomp($Line);
    my @Fields = split /\s+/,$Line;
    if(!defined($Fields[0]) || $Fields[0] =~ /NA/) { next; }
    my $Dstring = sprintf "%04d-%02d-%02d",$Fields[0],$Fields[1],$Fields[2];
    $Positions{$Dstring} = [$Fields[3],$Fields[4]];
}
close(DIN);

my @Imma;
open(DIN,"<obs.out.date.fmt") or die;
while(my $Line = <DIN>) {
    chomp($Line);
    $Line =~ s/\"//g;
