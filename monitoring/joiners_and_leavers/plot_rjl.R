...
ts<-read.table('by_day.txt')
ts$V1<-chron(dates=as.character(ts$V1),
             times="12:00:00",
             format=c(dates = "y-m-d", times = "h:m:s"))
pdf(file="rjl_per_day.pdf",
...
grid.yaxis(main=T)
grid.text('No. of volunteers',x=unit(-3.5,"lines"), rot=90)
...
grid.points(x=unit(ts$V1,"native"),y=unit(ts$V2,'native'),pch=20,
			               size=unit(3,"native"),gp=gp_black)
gp_blue  = gpar(col=rgb(0,0,1,1),fill=rgb(0,0,1,1))
grid.lines(x=unit(ts$V1,"native"),y=unit(ts$V3,'native'),
               gp=gp_blue)
grid.points(x=unit(ts$V1,"native"),y=unit(ts$V3,'native'),pch=20,
			               size=unit(3,"native"),gp=gp_blue)
gp_red  = gpar(col=rgb(1,0,0,1),fill=rgb(1,0,0,1))
grid.lines(x=unit(ts$V1,"native"),y=unit(ts$V4,'native'),
               gp=gp_red)
grid.points(x=unit(ts$V1,"native"),y=unit(ts$V4,'native'),pch=20,
