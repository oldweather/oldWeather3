...
png(file='imma.oW1.png',
  ...
  upViewport()
   ...
   pushViewport(viewport(x=unit(0,'npc'),y=unit(0.0,'npc'),
      ...
      tics<-pretty(as.POSIXct(dates))
      hist(as.POSIXct(dates),breaks='months',col='grey',freq=TRUE,main='',
           xlab='',ylab='No.of obs.',xaxt='n')
      axis(side=1,at=as.numeric(tics),labels=attr(tics,'labels'))
   upViewport()
  dev.off()
warnings()
